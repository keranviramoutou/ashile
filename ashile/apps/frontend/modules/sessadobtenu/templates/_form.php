...
<?php use_helper('Date') ?>
	...
	echo jq_form_remote_tag(array(
		'update' => 'div_sessad',
		'url' => 'sessadobtenu/' . ($form->getObject()->isNew() ? 'create' : 'update') . (!$form->getObject()->isNew() ? '?id=' . $form->getObject()->getId() . '&eleve_id=' . $form->getObject()->getEleveId() : '')))
	?>
	  ...
	  <table>
		...
		<tr>
			<td colspan="2" >
				<br />&nbsp;
				<?php if (!$form->getObject()->isNew()): ?>
					&nbsp;<?php echo jq_button_to_remote('Supprimer', array('url' => 'sessadobtenu/delete?id=' . $form->getObject()->getId() . '&eleve_id=' . $form->getObject()->getEleveId(), 'update' => 'div_sessad', 'confirm' => 'Vous êtes sur?')) ?>
				<?php endif; ?>
				<input type="submit" value="Enregistrer" onclick="return verif();" />
				<?php
					echo jq_button_to_remote('Revenir à la liste', array(
						'url' => 'sessadobtenu/index?id=' . $form->getObject()->getId() . '&eleve_id=' . $form->getObject()->getEleveId(),
						'update' => 'div_sessad',
					))
					?>
			</td>
		...
		<tbody>
		  ...
		  <tr>
					<th><?php echo $form['sessad_id']->renderLabel('Etablissement Sessad (*)') ?></th>
					<td>
...
<script>
  function verif() {
    ...
    d_2 = new Date(annee_2, mois_2-1, jour_2);
  	   ...
  	   if ($j("#sessadobtenu_datedebut").val() == "" || $j("#sessadobtenu_datefin").val() == "" ) {
         ...
         // return false;
		   ...
		   if( d_2 < d_1  ){  //date de début supérieure à la date de fin de scolarité
				...
				message = message + " - la date de début de prise en charge " + date1 + " supérieure à la date de fin de prise en charge " + date2  + " \n";
				 flag = 1;
				 $j("#sessadobtenu_datedebut").focus();
