...
<div class= 'aide' onClick="<?php echo url_for('specialiste/aide') ?>"></div> 
    ...
    <tfoot>
      <tr>
        <td colspan="2">
		  ...
		  <button type="button" onclick="location.href='<?php echo url_for('specialiste/index') ?>'">Retour</button>
          <?php if (!$form->getObject()->isNew()): ?>&nbsp;<?php echo button_to('Supprimer', 'specialiste/delete?id='.$form->getObject()->getId(),'confirm = etes vous sur ? popup=true') ?>
           <?php endif; ?>
          <input type="submit" value="Enregister"  onclick="return confirmation();" />
