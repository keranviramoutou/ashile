   ...
   $j(document).ready(function() {
       oTable = $j('#maTable').dataTable({
           ...
           "oLanguage" : {
               ...
               "sZeroRecords":    "Aucun Avs; afficher",
		...
		<tbody>
			<?php foreach ($contrat_avss as $contrat_avs): ?>
				<tr onClick="window.location='<?php echo url_for('contrat_avs/list?avs_id='.$contrat_avs['id']) ?>'">
				               <td><a href="<?php echo url_for('avs/edit?id='. $contrat_avs['id']) ?>"><?php echo $contrat_avs['nom'].'   '. $contrat_avs['prenom'] ?></a></td>
                               <td><?php echo format_date($contrat_avs['date_naissance'],'dd/MM/yyyy') ?>
							   <td> <?php echo link_to('Créer', 'contrat_avs/new?avs_id=' . $contrat_avs['id'] ) ?>
							   <?php echo link_to('Editer', 'contrat_avs/list?avs_id=' . $contrat_avs['id'] ) ?>
							   </td>
				</tr>
			<?php endforeach; ?>
		</tbody>
