	...
	<?php if ($sf_request->getParameter('eleve_id')) { ?>
       ...
       <?php if($count_dem_mat > 0){ ?>
		...
		<?php foreach ($demande_materiel as $demande_materiels){ ?>
			<?php echo  $demande_materiels['demandemateriel_id'].'- Type :&nbsp; <strong>'.$demande_materiels['typemateriel'].'&nbsp; cat :&nbsp;'.$demande_materiels['catmateriel'].'</strong>&nbsp;&nbsp;- Notifiée du &nbsp:&nbsp<strong>'.format_date($demande_materiels['datedebutnotif'],'dd/MM/yyyy').'</strong>&nbsp&nbspau&nbsp&nbsp<strong>'.format_date($demande_materiels['datefinnotif'],'dd/MM/yyyy')
	...
	<?php  echo $form->renderHiddenFields() ?>      
		   ...
		   <?php endif; ?>
			...
			<td>
				...
				<?php echo '-&ffffzzzzzzzzzzzzzzznbsp;'.$form['materiel_id']  ?>
			</td>
...
<script>
	...
	$j("#eleve_materiel_materiel_id") =  1233 ;
	titi = $j("#eleve_materiel_materiel_id").val();
	alert ( titi );   
